...
//        System.out.println("Exponenten: " + Arrays.toString(expo));
        zerlegeZahl(zahl);
        System.out.println(Arrays.toString(getPrimzahlenAlsArray()));
...
//        System.out.println("zahl: " + zahl);
        List<Long> primzahlen = new ArrayList<>();
...
//        System.out.println(Arrays.toString(primzahlenRelevant));
        for (int k = 0; k < primzahlenRelevant.length; k++) {
            if (zahl % primzahlenRelevant[k] == 0) {
                primzahlen.add(0L + primzahlenRelevant[k]);
                counter = 0L;
                do {
...
//            System.out.println("");
        ...
        primfaktorenUndExponten.add(primzahlen);
        primfaktorenUndExponten.add(exponenten);
        return primfaktorenUndExponten;
    }

    public long[] getPrimzahlenAlsArray() {
        Long[] primzahlenLong
    ...
    public void zeigeZahlDarstellung() {
        ...
        for (int i = 0; i < getPrimzahlenAlsArray().length; i++) {
            System.out.print(getPrimzahlenAlsArray()[i] + "^{" + getExponentenAlsArray()[i] + "}");
            if (i < getPrimzahlenAlsArray().length - 1) {
                System.out.print(" * ");
            } else {
                System.out.println("");
            }
    ...
    public void kontrolliereZahlDarstellung() {
        long sum = 1L;
        for (int i = 0; i < getPrimzahlenAlsArray().length; i++) {
            sum = sum * (long) Math.pow(getPrimzahlenAlsArray()[i], getExponentenAlsArray()[i]);
        }
